...
Starting a small frontend project with Webpack often means repeating the same setup: Sass, PostCSS, assets, fonts, linting, tests, aliases, a dev server, and a production build. This repository keeps that setup prepared in several variants, so you can copy the closest template and start from working defaults.
